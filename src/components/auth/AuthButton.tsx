...
type AuthButtonProps = {
  children: ReactNode;
  loading?: boolean;
  /** Shown instead of children while loading. Defaults to "Please wait…". */
  loadingLabel?: string;
...
export function AuthButton({
  ...
  const className =
    variant === "secondary"
      ? "inline-flex h-14 w-full items-center justify-center gap-2 rounded-2xl border border-border bg-surface px-5 text-sm font-semibold text-text shadow-soft transition hover:border-primary/25 hover:bg-card hover:shadow-lift focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-60"

...
export default function CandidatePlanDetailsPage({ params }: PageProps) {
  const { id } = use(params);
  ...
  const handleConfirmDeactivate = async () => {
    ...
    try {
      const res = await toggleCandidatePlanStatus(plan.id, false);
      if (res.success) {
        showToast(`Plan "${plan.name}" deactivated successfully.`);
  ...
  const handleConfirmActivate = async () => {
    ...
    setIsSubmittingAction(true);
    try {
      const res = await toggleCandidatePlanStatus(plan.id, true);
      if (res.success) {
        showToast(`Plan "${plan.name}" activated successfully.`);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[360px] space-y-3">
        ...
        <p className="text-xs text-text-muted font-medium">Loading candidate plan details...</p>
      </div>
    );
  }

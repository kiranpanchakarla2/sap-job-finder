...
export interface EmployerContactFormProps {
...
export function EmployerContactForm({
  ...
  // Filter category choices to only employer-relevant ones
  const employerCategories = CONTACT_REQUEST_CATEGORIES.filter((cat) =>
    (EMPLOYER_CONTACT_CATEGORIES as readonly string[]).includes(cat.value),
  );

  // Compute default pre-filled values
  const defaultName =
    companyProfile?.recruiterName ||
  ...
  if (!computedInitialSubject && contextJobTitle) {
  ...
  const {
    ...
    defaultValues: {
      name: defaultName,
      email: defaultEmail,
      category: initialCategory || (contextJobId ? "job_posting" : undefined),
  ...
  const onSubmit = async (values: ContactFormValues) => {
    ...
    // 1. Upload attachment to private employer storage if attached
    if (attachment.file) {
    ...
    // 2. Submit Contact Request to database service as authenticated employer
    ...
    return (
      ...
      >
        ...
        <p className="mx-auto mt-2 max-w-md text-xs text-muted leading-relaxed">
          We&apos;ll contact you using your registered email address ({defaultEmail || "your account email"}) if additional information is needed.
        </p>

        <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
          <Link
            href={EMPLOYER_ROUTES.dashboard}
    ...
    <div className="rounded-[var(--radius-card)] border border-border bg-card p-6 shadow-soft sm:p-8 lg:p-10">
        ...
        <div className="mb-6 flex items-start gap-3 rounded-xl border border-primary/20 bg-primary/5 p-4 text-xs sm:text-sm text-text">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted">
              Job: {contextJobTitle} {contextJobId && `(${contextJobId})`}
            </p>
          </div>
        </div>
      ) : null}

      {/* Company & Recruiter Identity Card */}
          ...
          <div className="flex items-center gap-3.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {companyProfile?.industry ? (
                <p className="text-xs text-muted truncate">{companyProfile.industry}</p>
              ) : null}
            </div>
          </div>

          <div className="flex items-center gap-2 rounded-full border border-border bg-card px-3.5 py-1.5 text-xs font-medium text-muted shrink-0 self-start sm:self-auto">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted">
          Our employer support specialists will review your inquiry and follow up promptly.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
        ...
        {/* Row 1: Employer Contact Name & Account Email */}
        <div className="grid gap-5 sm:grid-cols-2">
          {/* Contact / Recruiter Name Field */}
          <div>
            ...
            <input
              ...
              type="text"
              autoComplete="name"
              placeholder="e.g. Kiran Panchakarla"
              aria-required="true"
          ...
          {/* Email Field (Read-only for account consistency) */}
          <div>
            <div className="flex items-center justify-between">
              <label htmlFor={emailId} className="block text-sm font-medium text-text">
                Registered Work Email <span className="text-red-500" aria-hidden="true">*</span>
              </label>
            ...
            <p className="mt-1 text-[11px] text-muted">
              Replies will be sent to your registered company account email.
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <input
            id={subjectId}
            type="text"
            placeholder="e.g. Question regarding Excel bulk job upload validation"
            aria-required="true"
        ...
        <div>
          ...
          <textarea
            id={messageId}
            rows={5}
            placeholder="Please describe your question, issue, or request with relevant details…"
            aria-required="true"
        ...
        <div>
          ...
          {!attachment.file ? (
            <div
              ...
              role="button"
              tabIndex={0}
              aria-label="Upload an attachment. Click or drag and drop an Excel template, PDF, or screenshot."
            ...
            >
              ...
              <p className="mt-2 text-xs font-semibold text-text">
                Click to browse or drag and drop an Excel file, screenshot, or PDF
              </p>
              <p className="mt-1 text-[11px] text-muted">
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs text-muted flex items-center gap-1.5">
            ...
            <span>Authenticated under company session</span>
          </p>
        </div>
      </form>
    </div>

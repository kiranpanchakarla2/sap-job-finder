...
import { CONTACT_REQUEST_CATEGORIES, CONTACT_REQUEST_STATUSES } from "@/lib/constants";
...
export interface CandidateRequestHistoryProps {
  onNewRequest?: () => void;
}
...
export function CandidateRequestHistory({ onNewRequest }: CandidateRequestHistoryProps) {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      ...
      if (!res.success) {
        setError(res.error || "Failed to load support requests.");
        setRequests([]);
      } else {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted">
            Track inquiries, questions, and issue reports submitted from your candidate account.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-[var(--radius-card)] border border-border bg-card p-8 sm:p-12 text-center shadow-soft">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm text-muted">
            Have a question about your candidate profile, an application, a job posting, or your
            account? Send us a message and our support team will help.
          </p>
          {onNewRequest ? (
            <div className="mt-6">
              <button
            ...
            <div className="flex-1 overflow-y-auto py-4 space-y-4 text-sm">
              ...
              <div className="rounded-xl border border-primary/20 bg-primary/5 p-3.5 flex items-start gap-2.5 text-xs text-text">
                ...
                <p className="leading-relaxed text-muted">
                  Our support team reviews requests in the order received. Responses are sent to your

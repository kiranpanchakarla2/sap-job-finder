      ...
      if (plansRes.data && plansRes.data.length > 0) {
        plans = plansRes.data.map((row) => {
          ...
          return {
            ...
            limits: {
              activeJobs: row.max_active_jobs !== undefined ? row.max_active_jobs : (PLAN_LIMITS[id]?.activeJobs ?? fallback?.limits?.activeJobs ?? 5),
              applications: row.max_applications !== undefined ? row.max_applications : (PLAN_LIMITS[id]?.applications ?? fallback?.limits?.applications ?? 50),
              talentSearch: row.max_talent_search !== undefined ? row.max_talent_search : (PLAN_LIMITS[id]?.talentSearch ?? fallback?.limits?.talentSearch ?? 10),
              teamMembers: row.max_team_members !== undefined ? row.max_team_members : (PLAN_LIMITS[id]?.teamMembers ?? fallback?.limits?.teamMembers ?? 1),
            },
            featureFlags: Array.isArray(row.feature_flags) && row.feature_flags.length > 0 ? (row.feature_flags as PlanDefinition["featureFlags"]) : (PLAN_FEATURES[id] || fallback?.featureFlags || []),
            highlighted: row.highlighted !== undefined ? Boolean(row.highlighted) : (id === "pro"),
            badge: row.badge ?? (id === "pro" ? "Most Popular" : id === "business" ? "Best Value" : undefined),

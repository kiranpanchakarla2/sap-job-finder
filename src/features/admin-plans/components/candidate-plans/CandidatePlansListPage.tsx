...
import {
  fetchCandidatePlans,
  toggleCandidatePlanStatus,
} from "../../services/adminCandidatePlanService";
import { CandidatePlansTable } from "./CandidatePlansTable";
import { CandidatePlanDeactivateModal } from "./CandidatePlanDeactivateModal";
import { CandidatePlanActivateModal } from "./CandidatePlanActivateModal";

export function CandidatePlansListPage() {
  const [plans, setPlans] = useState<AdminCandidatePlan[]>([]);
  ...
  // Modals state
  const [selectedPlan, setSelectedPlan] = useState<AdminCandidatePlan | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetchCandidatePlans({
        filters,
        sortBy,
        sortOrder,
      });
      ...
      setError(err instanceof Error ? err.message : "Failed to load candidate plans");
    } finally {
      setIsLoading(false);
    }
  ...
  const handleConfirmDeactivate = async () => {
    ...
    try {
      const res = await toggleCandidatePlanStatus(selectedPlan.id, false);
      if (res.success) {
        showToast(`Plan "${selectedPlan.name}" deactivated successfully.`);
  ...
  const handleConfirmActivate = async () => {
    ...
    setIsSubmittingAction(true);
    try {
      const res = await toggleCandidatePlanStatus(selectedPlan.id, true);
      if (res.success) {
        showToast(`Plan "${selectedPlan.name}" activated successfully.`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-text-muted mt-1">
            Create, configure, edit, and activate subscription tiers offered to candidates.
          </p>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-surface p-3.5 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-soft">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            placeholder="Search candidate plans..."

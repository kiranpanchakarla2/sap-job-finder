...
import { CANDIDATE_FEATURE_OPTIONS } from "../../constants/candidateFeatures";

type CandidatePlanDetailsViewProps = {
  plan: AdminCandidatePlan;
  onDeactivate: () => void;
  onActivate: () => void;
};

export const CandidatePlanDetailsView = memo(function CandidatePlanDetailsView({
  plan,
  onDeactivate,
  onActivate,
}: CandidatePlanDetailsViewProps) {
    ...
    <div className="space-y-8 max-w-5xl pb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="rounded-2xl border border-border bg-surface p-6 space-y-4">
          ...
          <div className="space-y-3 text-xs">
            <div className="flex justify-between items-center py-1.5 border-b border-border/50">
              <span className="text-text-muted">Applications / mo</span>
              <span className="font-bold text-text">
                {plan.limits.applicationsPerMonth === null ? "Unlimited" : plan.limits.applicationsPerMonth}
              </span>
            </div>
            <div className="flex justify-between items-center py-1.5 border-b border-border/50">
              <span className="text-text-muted">Active Job Alerts</span>
              <span className="font-bold text-text">
                {plan.limits.jobAlerts === null ? "Unlimited" : plan.limits.jobAlerts}
              </span>
            </div>
            <div className="flex justify-between items-center py-1.5 border-b border-border/50">
              <span className="text-text-muted">Saved Jobs Limit</span>
              <span className="font-bold text-text">
                {plan.limits.savedJobs === null ? "Unlimited" : plan.limits.savedJobs}
              </span>
            </div>
            <div className="flex justify-between items-center pt-1">
              <span className="text-text-muted">Resume Versions</span>
              <span className="font-bold text-text">
                {plan.limits.resumeVersions === null ? "Unlimited" : plan.limits.resumeVersions}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-surface p-6 space-y-4">
        <div className="flex items-center gap-2 text-text font-bold text-sm border-b border-border pb-3">
          ...
          Feature Capabilities Entitlement
        </div>
      ...
      {/* Candidate-Facing Display Bullets */}
      <div className="rounded-2xl border border-border bg-surface p-6 space-y-4">
        <div className="flex items-center gap-2 text-text font-bold text-sm border-b border-border pb-3">
          <FileText className="h-4 w-4 text-amber-500" />
          Candidate-Facing Feature Highlights
        </div>

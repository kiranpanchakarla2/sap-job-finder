...
type EmployerPlanDeactivateModalProps = {
...
export const EmployerPlanDeactivateModal = memo(function EmployerPlanDeactivateModal({
  plan,
  isOpen,
  isSubmitting,
  onClose,
  onConfirm,
}: EmployerPlanDeactivateModalProps) {
  useEffect(() => {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="deactivate-employer-plan-title"
    >
      <div className="w-full max-w-md rounded-2xl bg-surface border border-border shadow-elevation-3 p-6 space-y-5">
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            <h3 id="deactivate-employer-plan-title" className="text-lg font-bold text-text">
              Deactivate Plan?
            </h3>
        ...
        {plan.activeSubscriptionsCount !== undefined && plan.activeSubscriptionsCount > 0 && (
          <div className="text-xs text-text-muted flex items-center gap-1.5 px-1">
            ...
            employer subscription{plan.activeSubscriptionsCount === 1 ? "" : "s"}.
          </div>
        )}

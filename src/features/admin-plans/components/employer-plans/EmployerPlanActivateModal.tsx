...
type EmployerPlanActivateModalProps = {
  plan: AdminEmployerPlan | null;
  isOpen: boolean;
...
export const EmployerPlanActivateModal = memo(function EmployerPlanActivateModal({
  plan,
  isOpen,
  isSubmitting,
  onClose,
  onConfirm,
}: EmployerPlanActivateModalProps) {
  useEffect(() => {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="activate-employer-plan-title"
    >
      <div className="w-full max-w-md rounded-2xl bg-surface border border-border shadow-elevation-3 p-6 space-y-5">
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            <h3 id="activate-employer-plan-title" className="text-lg font-bold text-text">
              Activate Plan?
            </h3>
        ...
        <div className="rounded-xl bg-surface-hover/60 border border-border/70 p-3.5 text-sm text-text-secondary leading-relaxed">
          Make this plan available for new subscriptions again? It will immediately appear on the Employer subscription page in its configured display order.
        </div>

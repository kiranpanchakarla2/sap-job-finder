...
import type { EmployerPaginationState } from "../../types/employer.types";

type EmployerPaginationProps = {
...
export const EmployerPagination = memo(function EmployerPagination({
  pagination,
  onPageChange,
  onPageSizeChange,
  isLoading = false,
}: EmployerPaginationProps) {
    ...
    <div className="flex flex-wrap items-center justify-between gap-4 border-t border-border bg-surface/30 px-5 py-3.5 text-xs text-muted">
      <div className="flex items-center gap-4">
        <span>
          ...
          <span className="font-semibold text-text">{totalItems}</span> employers
        </span>

        <div className="flex items-center gap-1.5">
          <label htmlFor="employer-page-size" className="text-muted">
            Per page:
          </label>
          <select
            id="employer-page-size"
            value={pageSize}

...
type EmployerReactivateModalProps = {
  isOpen: boolean;
  companyName: string;
  onClose: () => void;
...
export function EmployerReactivateModal({
  isOpen,
  companyName,
  onClose,
  onConfirm,
}: EmployerReactivateModalProps) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="reactivate-employer-title"
      ...
      <div className="relative w-full max-w-md rounded-[var(--radius-card)] border border-border bg-card p-6 shadow-xl space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="reactivate-employer-title" className="text-base font-semibold text-text">
                Reactivate Employer?
              </h2>
              <p className="text-xs text-muted">Restore Company Account Access</p>
            </div>
          </div>
        ...
        <div className="space-y-2 text-xs text-muted leading-relaxed">
          ...
          <p className="text-xs text-text">
            This will restore Employer Portal access for the company owner and active company team members.
          </p>
        </div>

-- Sprint 7D: Bulk Job Database Import RPC with duplicate detection and tenant authorization

create or replace function public.bulk_import_jobs(p_jobs jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  -- 4. Check Employer Role Authorization (owner, admin, or recruiter)
  ...
  if v_role is null then
    -- Fallback for founding company profile
    if exists (
  ...
  loop
    ...
    if v_title = '' then
      v_failed := v_failed || jsonb_build_object(
        ...
        'jobTitle', coalesce(nullif(v_title, ''), 'Untitled Job'),
    ...
    -- =========================================================================
    -- DUPLICATE DETECTION (Company-Scoped & Intra-Batch)
    -- =========================================================================
    -- Check if already seen in current import batch
    ...
    -- Check against existing jobs in database for this company
    v_existing_id := null;
...
revoke all on function public.bulk_import_jobs(jsonb) from public, anon;
grant execute on function public.bulk_import_jobs(jsonb) to authenticated;
